...
public class BookingController {
    ...
    @PostMapping
    public BookingOutDto create(@RequestHeader(USER_HEADER) Long userId, @RequestBody BookingDto bookingDto) {
        log.info("Request for new booking for userId: {}", userId);
        return bookingService.add(userId, bookingDto);
    }

    @PatchMapping("/{bookingId}")
    public BookingOutDto updateStatus(@RequestHeader(USER_HEADER) Long userId,
    ...
    public BookingOutDto findBookingById(@RequestHeader(USER_HEADER) Long userId,
                                         @PathVariable("bookingId")
                                         Long bookingId) {
    ...
    @GetMapping
    public List<BookingOutDto> findAllForUser(@RequestHeader(USER_HEADER) Long userId,
    ...
    @GetMapping("/owner")
    public List<BookingOutDto> getAllOwnerBookings(@RequestHeader(USER_HEADER) Long ownerId,
                                                   @RequestParam(value = "state", defaultValue = "ALL") String bookingState,
                                                   @RequestParam(required = false) @Min(0) Integer from,
                                                   @RequestParam(required = false) @Min(1) Integer size) {
